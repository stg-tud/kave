...
namespace KaVE.VS.Achievements.LogCollector
{
    ...
    public class LogReplay
    {
        ...
        public void CollectEventsFromLog(BackgroundWorker worker)
        {
            ...
            // Reloads Achievement UI after blocking its updates (see ReloadAchievements method)
            var achievementWindowControl = Registry.GetComponent<IUiDelegator>().AchievementUserControl;
            if (achievementWindowControl != null)
            {
                achievementWindowControl.ReloadAchievements(null, null);
            }
        }
