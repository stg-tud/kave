...
{
    internal class FieldNameTest : MemberTestBase
    {
        protected override IMemberName GetMemberNameForBaseTests(string basicMemberSignature)
        {
            return new FieldName(basicMemberSignature);
        }

        [Test]
        public void DefaultValues()
        {
            var sut = new FieldName();
        ...
        {
            Assert.True(new FieldName().IsUnknown);
            Assert.True(new FieldName("[?] [?].???").IsUnknown);
            Assert.False(new FieldName("[T1,P] [T2,P].f").IsUnknown);
        }
