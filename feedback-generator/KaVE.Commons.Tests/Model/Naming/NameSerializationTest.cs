...
namespace KaVE.Commons.Tests.Model.Naming
{
    internal class NameSerializationTest
    {
        ...
        {
            Assert.True(expectedType.IsInstanceOfType(NameSerialization.ParseJson(input)));
        }
        ...
        {
            var name = NameSerialization.ParseJson(input);
            Console.WriteLine(name);
            Assert.AreEqual(expected, NameSerialization.ToJson(name));
        }
