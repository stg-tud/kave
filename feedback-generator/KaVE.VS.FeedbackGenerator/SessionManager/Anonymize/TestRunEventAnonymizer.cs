...
{
    internal class TestRunEventAnonymizer : IDEEventAnonymizer
    {
        public override void AnonymizeCodeNames(IDEEvent ideEvent)
        {
            base.AnonymizeCodeNames(ideEvent);

            var e = ideEvent as TestRunEvent;
            if (e != null)
            {
                foreach (var test in e.Tests)
                {
                    test.Parameters = test.Parameters.ToHash();
                    test.TestMethod = test.TestMethod.ToAnonymousName();
                }
            }
        }
    }
