...
{
    internal class ErrorAndInfoEventFilterTest
    {
        private ErrorAndInfoEventFilter _sut;

        [SetUp]
        public void Setup()
        {
            _sut = new ErrorAndInfoEventFilter();
        }

        [Test]
        public void ShouldPreserveEventsWithNonEmptySid()
        {
            Assert.True(_sut.Func2(new CommandEvent()));
        }

        [Test]
        public void ShouldRemoveEventsWithoutSid_Empty()
        {
            Assert.False(_sut.Func2(new InfoEvent()));
        }

        [Test]
        public void ShouldRemoveEventsWithoutSid_Null()
        {
            Assert.False(_sut.Func2(new ErrorEvent()));
        }
    }
}